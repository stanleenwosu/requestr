...
import { Bid, IStatus } from "app/@core/data/order";
...
export class PlaceBidComponent implements OnInit {
  ...
  async placeBid(fileUrl: string = "") {
    ...
    try {
      ...
      await this.notS.addBidNotification(
        "New",
        `New Bid for Order ${this.orderId} has been placed`,
        this.orderId,
        bid.id,
        "staffs"
      );
      this.toastr.success(`New Bid Added`, "Bid Success", {
        duration: 2000,
        position: this.tp,
      });
      this.dialogRef.close(bid);
    } catch (error) {
      this.toastr.danger(error, "Bid Error", {
        duration: 2000,
        position: this.tp,
      });
    }
  ...
  uploadToDB() {
    ...
    const uploadTask = firebase
      .storage()
      .ref()
      .child(`files/${this.fileInfo.name}`)
      .putString(this.filesToUpload, "data_url");
    uploadTask.on(
      "state_changed",
      (snapshot) => {
